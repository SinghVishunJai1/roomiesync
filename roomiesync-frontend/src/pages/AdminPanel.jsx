...
import { UserPlus, ClipboardList, ShieldAlert, Trash2, Users, CheckCircle2, AlertCircle, X, AlertTriangle } from 'lucide-react';
...
export default function AdminPanel() {
        ...
        return (
            <div className="flex h-screen items-center justify-center bg-neutral-950 p-8 text-neutral-100">
                <div className="bg-black/40 border border-neutral-800/80 p-8 rounded-3xl shadow-2xl backdrop-blur-xl text-center space-y-3 max-w-md w-full">
                    ...
                    <p className="text-sm text-neutral-400">Only Admin can view this page.</p>
                </div>
            </div>
        );
    }
    ...
    const handleAssignTask = async (e) => {
        e.preventDefault();
        try {
            await api.post('/tasks/assign', { title: taskTitle, assignedToUserId: parseInt(assignedUserId) });
            showToast('Task assigned successfully!', 'success');
            setTaskTitle(''); setAssignedUserId('');
        } catch (err) {
        ...
        >
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                        ...
                        >
                            ...
                            <div className="text-center space-y-1">
                                <h3 className="text-lg font-bold text-neutral-100">Delete Flatmate?</h3>
                                <p className="text-xs text-neutral-400">
                                    Are you sure you want to remove <span className="text-neutral-200 font-semibold">{userToDelete.name}</span>? This action cannot be undone.
                                </p>
                            </div>
            ...
            <div className="bg-black/40 border border-neutral-800/80 p-6 rounded-3xl shadow-xl backdrop-blur-xl space-y-4">
                <h2 className="text-lg font-bold text-neutral-200 flex items-center gap-2">
                    <Users className="text-rose-400" size={20} /> Manage Flatmates (Delete Users)
                </h2>
                <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                    {usersList.map((u) => (
                        <div key={u.id} className="p-4 border border-neutral-800 rounded-2xl flex justify-between items-center bg-neutral-900/60">
                            <div>
                                <p className="font-bold text-neutral-200 text-sm flex items-center gap-2">
                                    {u.name} <span className="text-[10px] text-neutral-500 font-mono">ID: #{u.id}</span>
                                </p>
